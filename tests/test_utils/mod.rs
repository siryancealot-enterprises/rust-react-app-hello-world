...
use rust_react_app_hello_world::services::{self, search::get_test_search_client};

pub fn get_test_server_with_app(pool: sqlx::PgPool) -> axum_test::TestServer {
    let router: axum::Router = services::app_server::init_router(pool, get_test_search_client());
