...
async fn run_js(file_path: &str) -> Result<(), AnyError> {
    let mut path_with_scripts_dir: String = "./ts_scripts/".to_string();
    path_with_scripts_dir.push_str(file_path);

    let main_module =
        deno_core::resolve_path(&path_with_scripts_dir, env::current_dir()?.as_path())?;
...
pub fn execute_typescript(file_path: &str) {
